...
1. `val`로 들어온 값으로 `Node`를 생성한다. 그리고 `head`와 `tail`에 새로생성한 노드를 할당한다.
   ...
   ![단일 연결 리스트 데이터구조1](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList1.webp)

2. 나머지 경우에는 이미 `Node`가 존재하고 있기 때문에 새로 만든 노드를 `tail`의 `next` 부분에 추가한다.
3. 그리고 기존에 있던 tail의 value를 새로운 노드로 변경하고 길이를 증가시킨다.

![단일 연결 리스트 데이터구조2](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList2.webp)
...
2. `pop()` 메서드는 왼쪽에 `head` 노드부터 리스트를 순회하며 `tail`의 **이전** `Node`를 찾는다. 그렇기 때문에 **빅오(N)의 값을 갖는다.**
   ...
   ![단일 연결 리스트 데이터구조3](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList3.webp)

3. `tail` 이전의 값을 `newTail` 변수에 할당하고 이전에 연결 되어있었던 `tail`의 값을 `null`을 할당에 끊는다.

   ![단일 연결 리스트 데이터구조4](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList4.webp)
...
2. 현재 인스턴스에 `head`의 값을 `currentHead` 변수에 할당하고 현재 `head`에는 연결되어있는 다음 `Node`를 할당한다.
   ...
   ![단일 연결 리스트 데이터구조5](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList5.webp)

   ![단일 연결 리스트 데이터구조6](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList6.webp)
...
1. 우선 `val`로 연결 시키고자 하는 `value`를 인자로 받는다.
   ...
   ![단일 연결 리스트 데이터구조7](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList7.webp)
...
3. 그게 아니라면 새로운 `Node.next`에 이전에 `head` 값을 연결하고 `head`에 새로운 `Node`를 추가한다.
   ...
   ![단일 연결 리스트 데이터구조8](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList8.webp)
...
3. 원하는 위치에 `Node`를 `insert`하기 위해선 `index-1번째 Node`를 찾고 `index-1번째  Node.next`를 `temp` 변수에 할당한다.
   ...
   ![단일 연결 리스트 데이터구조9](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList9.webp)

4. `temp`를 새로만든 `Node.next`에 할당하고 `prevNode.next`에 새로운 `Node`를 할당한다.

   ![단일 연결 리스트 데이터구조10](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList10.webp)

   ![단일 연결 리스트 데이터구조11](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList11.webp)
...
2. preNode변수에 index - 1 번째 Node를 get 메서드를 통해 찾아 할당하고 preNode.next의 값을 removed 변수에 할당한다.
   ...
   ![단일 연결 리스트 데이터구조12](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList12.webp)

3. removed.next에는 index + 1번째 Node가 있기 때문에 prevNode.next에 할당해 연결 시켜준다.

   ![단일 연결 리스트 데이터구조13](https://raw.githubusercontent.com/thdtjdgml415/next-blog/main/assets/img/data-structure/singleList/singleList13.webp)
